...
class Source:  # labels
  ...
  NUMBER_IMAGES = 'spoken_numbers.zip'  # width=256 height=256
...
def mfcc_batch_generator(batch_size=10, source=Source.DIGIT_WAVES, target=Target.digits):
  ...
  while True:
    ...
    for wav in files:
      ...
      mfcc=np.pad(mfcc,((0,0),(0,88-len(mfcc[0]))), mode='constant', constant_values=0)
      batch_features.append(np.array(mfcc))
      if len(batch_features) >= batch_size:
        # print(np.array(batch_features).shape)
        # yield np.array(batch_features), labels
        return np.asarray(batch_features), np.asarray(labels)  # basic_rnn_seq2seq inputs must be a sequence
